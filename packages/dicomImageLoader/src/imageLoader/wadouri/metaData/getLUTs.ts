...
function getLUT(pixelRepresentation: number, lutDataSet: DataSet): Types.LutType {
...
function getLUTs(pixelRepresentation: number, lutSequence: Element): Types.LutType[] {
  if (!lutSequence || !lutSequence.items || !lutSequence.items.length) {
    return;
  }
  const luts: Types.LutType[] = [];
